<?php
/**
 * Divi 5 REST Controller: GS Team Members preview HTML.
 *
 * @package GSTEAM
 */

namespace GSTEAM\Divi;
...
class TeamMembersController {
	...
	public static function permissions_check() {
		if ( ! is_user_logged_in() ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You must be logged in to preview GS Team shortcodes.', 'gsteam' ),
				[ 'status' => 401 ]
			);
		}
		...
		return new WP_Error(
			'rest_forbidden',
			__( 'Sorry, you are not allowed to preview GS Team shortcodes.', 'gsteam' ),
			[ 'status' => 403 ]
		);
	}
	...
	public static function render( WP_REST_Request $request ) {
		...
		if ( ! $shortcode_id ) {
			$shortcode_id = absint( TeamMembersModule::get_default_shortcode_id() );
		}
		...
		if ( ! $shortcode_id ) {
			return rest_ensure_response(
				[
					'html'         => sprintf(
						'<div class="gs-team-divi-empty">%s</div>',
						esc_html__( 'No shortcodes found. Create one in GS Team first.', 'gsteam' )
					),
					'shortcode_id' => 0,
				]
			);
		...
		try {
			$html = TeamMembersModule::render_shortcode_html( $shortcode_id );
		} catch ( \Throwable $exception ) {
			return new WP_Error(
				'gs_team_divi_render_failed',
				sprintf(
					/* translators: %s: error message */
					__( 'Failed to render shortcode: %s', 'gsteam' ),
